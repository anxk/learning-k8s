...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='224be7bdce5a9dd0c2fd0d46b83865648e2fe0ba'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'tag: v1.16.5-beta.0, tag: v1.16.4' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'tag: v1.16.5-beta.0, tag: v1.16.4' =~ tag:\ (v[^ ,]+) ]]; then
